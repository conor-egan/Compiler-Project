...
PRIVATE void ParseDeclarations( void )
{
  ...
  while ( CurrentToken.code == COMMA )
     {
        Accept( COMMA );
	MakeSymbolTableEntry(STYPE_VARIABLE, NULL); /*Make variable entry*/
        Accept( IDENTIFIER );
		vcount++;
    }
  Accept( SEMICOLON );
...
PRIVATE void ParseProcDeclaration( void )
{
  ...
  SYMBOL *procedure;
  Accept( PROCEDURE );
  procedure = MakeSymbolTableEntry( STYPE_PROCEDURE, NULL );/*Make symbol table entry*/
	...
	switch ( CurrentToken.code ) {
		...
		case SEMICOLON:
			if ( target != NULL && target->type == STYPE_PROCEDURE ) {
				...
				KillCodeGeneration();/*Print Error and kill Code generation*/
			}
			break; 
...
/*                                                                          */
/*--------------------------------------------------------------------------*/
/*COMPOUNDTERM IS NOT USED IN COMPILER*/
PRIVATE void CompoundTerm( void )
{
	int op;
	Term();
...
PRIVATE void Term( void )
{
	int negateflag = 0;/*Flag to determine if term is negative*/
    ...
    case IDENTIFIER:
	  default:
	    ...
	    if ( var != NULL && var->type == STYPE_VARIABLE ) {
            if (writing) {/*Flag handlers for READ and WRITE*/
		...
		else {/*Print error and kill code generation*/
...
{
  Term();
  ParseOpPrec(0);  /*Call to parse term and operator precedence*/
}
